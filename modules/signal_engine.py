...
def create_signal(strategies: list[dict], gold_analysis: dict, macro: dict, mtf: dict, config: dict, event_risk: dict, trade_status: dict | None = None) -> dict:
    ...
    if conflict:
        reason = "Không nên chống lại giá. Chờ xác nhận reject hoặc breakdown mới giao dịch."
    ...
    signal = {
        ...
        "risk_level": "Cao" if event_risk.get("blocked") or macro.get("score", 0) >= 81 else "Trung bình",
        ...
        "reason": reason,
        "allow_auto_trade": bool(config.get("trade", {}).get("allow_auto_trade", False) and action in {"BUY", "SELL"}),
        "conflict": conflict,
